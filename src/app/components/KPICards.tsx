...
export function KPICards({ total, pendentes, aprovados, uti, enfermaria, hemodinamica, transInterHosp, outros }: Props) {
  ...
  const donutData = [
    { value: taxaAprovacao },
    { value: 100 - taxaAprovacao },
  ];

  return (
    <div className="grid grid-cols-5 gap-2 mb-2">
      {/* Solicitações Reguladas */}
      ...
      <div className="bg-card border border-white/8 rounded p-3">
        ...
        <div className="grid grid-cols-5 gap-1">
          {[
            { label: "UTI", value: uti, icon: "🛏" },
            { label: "Enfermaria", value: enfermaria, icon: "🛏" },
            { label: "Hemodinâmica", value: hemodinamica, icon: "❤" },
            { label: "Trans. Inter-Hosp", value: transInterHosp, icon: "🚑" },
