...
export function ApprovalRateTable({ hospitals }: Props) {
  ...
  const total = {
    hospital: "TOTAL",
    solicitacoes: hospitals.reduce((s, h) => s + h.solicitacoes, 0),
    aprovacoes: hospitals.reduce((s, h) => s + h.aprovacoes, 0),
    taxa: 0,
  };
        ...
        {sorted.map((h, i) => {
          ...
          return (
            <Fragment key={i}>
              ...
              <div className="py-1.5 border-b border-white/5 text-center">
                <RateBadge value={taxa} />
              </div>
            </Fragment>
          );
        })}
